...
if not openai_api_key:
    ...
    api_client = ApiService(api_key=openai_api_key)
...
def evaluate_gdpr_breach():
    ...
    evaluation_result = api_client.get_case_evaluation(case_description)
...
@evaluate_bp.route('/summarize', methods=['POST'])
def summarize():
    """
    API endpoint to evaluate a GDPR data breach case using OpenAI.
    Expects a JSON payload with a 'case_description' field.
    ...
    evaluation_result = api_client.summarize_description(case_description)
